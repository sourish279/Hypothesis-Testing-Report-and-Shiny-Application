...
5. **p-value:**  $P(\chi^2_{1} \ge 17.6) = 1.726444 * 10 ^{-5}$
\

```{r, echo=FALSE}
1-pchisq(test$statistic, df = 1)
```
6. **Decision:**  At a  0.05 significant level, we reject the null hypothesis. The data is not consistent with the null hypothesis that the data follow a Poisson distribution.
...
```{r, warning=FALSE, fig.width= 6.5, fig.cap= "Figure 4: Comparative violin plots with boxplot overlay for heights by gender."}
fig <- df %>%
  plot_ly(
    x = ~gender,
    y = ~height,
